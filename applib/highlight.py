...
class HighlightBehavior(object):
    ...
    current_highlighted_child = ObjectProperty(None, allownone=True) # ''' current highlighted child on ObjectProperty and defualts to None '''
    highlighted_color = ListProperty([.2,.5,1,.0]) # ''' color that show highlighted widget a ListProperty defualts to [.2,.5,1.5] make sure it transparent because it's drawn over the widget '''
    auto_scroll_to = BooleanProperty(False) # ''' automatically scroll to the widget when widget is out of focus note* parent most be a scrollview to enable the property '''
    highlighted_shape = OptionProperty('rounded_rectangle', options=['rounded_rectangle', 'rectangle'])
    highlight_orientation = OptionProperty('vertical', options=['vertical', 'horizontal', 'grid']) # ''' Orientation in which the highlighting will take place if grid grid len must be set '''
    instruction_canvas = ObjectProperty(InstructionGroup()) # ''' internal instruction group used to draw the canvas on the currently highlighted child '''
