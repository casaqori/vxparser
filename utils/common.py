...
def Callback(key, value):
    global client
    osc_client(b'/cb', key, value)


def Logger(lvl, msg, name=None, typ=None):
    if int(lvl) == 0:
        if name and typ:
            osc_client(b'/echo', msg, name, typ)
        elif name:
            osc_client(b'/echo', msg, name)
        elif typ:
            osc_client(b'/echo', msg, typ)
        else:
            osc_client(b'/echo', msg)
    elif int(lvl) == 9:
        osc_client(b'/stat', name, typ, msg)
    else:
        if name and typ:
            osc_client(b'/log', str(lvl), msg, name, typ)
        elif name:
            osc_client(b'/log', str(lvl), msg, name)
        elif typ:
            osc_client(b'/log', str(lvl), msg, typ)
        else:
            osc_client(b'/echo', msg)
    return


def osc_server(type, *values):
    val = []
    if values and len(values) > 0:
        for v in values:
            val.append(v.encode('utf8'))
    server.send_message(type, val, *client.getaddress())


def osc_client(type, *values):
    global client
    val = []
    for v in values:
        val.append(v.encode('utf8'))
